...
public class PodTrackingServiceTests {
    ...
    @Autowired
    PodTrackingService podTrackingService;
    ...
    public void testHandle() {
        podTrackingService.handle(template);
        await()
                .atMost(5, TimeUnit.SECONDS)
                .untilAsserted(() -> verify(template, Mockito.atLeastOnce()).convertAndSend(Mockito.eq(Topics.POD),
                        Mockito.any(V1PodList.class)));
    }
}
